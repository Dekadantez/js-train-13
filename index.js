...
function searchParamsURL(url) {
  ...
  for (const el of URLSearchParams) {
    param.set(el[0], el[1]);
  }
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (let k of key) {
    urlObj.searchParams.append(k, paramsObj[k]);
  }
...
function deleteSearchParams(keys, url) {
  ...
  for (let newKeys of keys) {
    urlObj.searchParams.delete(newKeys);
  }
  // Повертаємо нову URL-адресу в рядковому форматі.
  return urlObj.href.toString();
}
...
function processUrl(url, options) {
  ...
  const param = options.searchParams;
  for (let elem in param) {
    urlObj.searchParams.append(elem, param[elem]);
...
function getUrlData(url) {
  ...
  return {
    origin: urlObj.origin, //походження URL.
    hostname: urlObj.hostname, //ім'я хоста URL.
    port: urlObj.port, //порт URL.
    username: urlObj.username, //ім'я користувача в URL.
    password: urlObj.password, //пароль в URL.
  };
}
